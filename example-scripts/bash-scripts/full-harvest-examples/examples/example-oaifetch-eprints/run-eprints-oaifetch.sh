...
COMMON_CONFIG_DIRECTORY=../common
...
export HARVEST_NAME=EPrints-OAI-fetch
...
java $HARVESTER_JAVA_OPTS org.vivoweb.harvester.fetch.OAIFetch -X eprints-oaifetch.conf.xml
...
harvester-transfer -w INFO -s $COMMON_CONFIG_DIRECTORY/translation/translated-records.config.xml -o $COMMON_CONFIG_DIRECTORY/harvestertransfer/harvested-data.model.xml -d data/harvested-data/imported-records.rdf.xml
...
harvester-transfer -w info -o $COMMON_CONFIG_DIRECTORY/harvestertransfer/previous-harvest.model.xml -r data/vivo-subtractions.rdf.xml -m
# Apply Additions to Previous model
harvester-transfer -w info -o $COMMON_CONFIG_DIRECTORY/harvestertransfer/previous-harvest.model.xml -r data/vivo-additions.rdf.xml
...
if [[ "$1" == "tdb" ]]; then
  ...
  harvester-transfer -w info -o $COMMON_CONFIG_DIRECTORY/modelupdate/vivo.model.xml -r data/vivo-subtractions.rdf.xml -m
  # Apply Additions to VIVO model
  harvester-transfer -w info -o $COMMON_CONFIG_DIRECTORY/modelupdate/vivo.model.xml -r data/vivo-additions.rdf.xml
fi

if [[ "$1" == "" || "$1" == "sparql" ]]; then
  java $HARVESTER_JAVA_OPTS org.vivoweb.harvester.services.SparqlUpdate -X $COMMON_CONFIG_DIRECTORY/modelupdate/sparqlupdate.conf.xml
fi

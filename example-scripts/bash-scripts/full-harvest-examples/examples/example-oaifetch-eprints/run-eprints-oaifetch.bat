...
REM Note: mklink requires admin privileges on Windows, use copy as fallback
...
echo Fetching from EPrints...
IF NOT EXIST eprints-oaifetch.conf.xml (
    echo ERROR: Missing configuration file: eprints-oaifetch.conf.xml
    exit /b 1
)
java %HARVESTER_JAVA_OPTS% -cp "%CLASSPATH%" org.vivoweb.harvester.fetch.OAIFetch -X eprints-oaifetch.conf.xml
...
REM Note: harvester-xsltranslator is a shell script, use Java directly
